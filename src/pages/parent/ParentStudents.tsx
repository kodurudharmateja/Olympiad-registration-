...
export default function ParentStudents() {
  ...
  const createMutation = trpc.student.createByParent.useMutation({
    onSuccess: () => {
      utils.student.listByParent.invalidate();
      setIsAddOpen(false);
      setNewStudent({ name: "", className: "", section: "", gender: "Male", mobile: "", parentName: "", schoolName: "" });
    },
  });
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-xl font-semibold text-[#2D2D2D]">My Children</h2>
          <p className="text-sm text-[#6B6560]">Manage your children's profiles</p>
        </div>
        <Dialog open={isAddOpen} onOpenChange={setIsAddOpen}>
          ...
          <DialogContent className="bg-white border-[#E8E4E0]">
            <DialogHeader>
              <DialogTitle className="text-[#2D2D2D]">Add Your Child</DialogTitle>
            </DialogHeader>
            <form onSubmit={(e) => { e.preventDefault(); createMutation.mutate(newStudent); }} className="space-y-3">
              ...
              <div className="space-y-2">
                ...
                <Select value={newStudent.gender} onValueChange={(v) => setNewStudent({ ...newStudent, gender: v })}>
                  <SelectTrigger className="border-[#D9D4CC]"><SelectValue /></SelectTrigger>
                  <SelectContent>
              ...
              <div className="space-y-2">
                <Label className="text-[#2D2D2D]">School Name (Optional)</Label>
                <Input value={newStudent.schoolName} onChange={(e) => setNewStudent({ ...newStudent, schoolName: e.target.value })} className="border-[#D9D4CC]" placeholder="Current school of your child" />
              </div>
      ...
      <div className="relative">
        ...
        <Input placeholder="Search children..." value={search} onChange={(e) => setSearch(e.target.value)} className="pl-9 border-[#D9D4CC] bg-white" />
      </div>
              ...
              <tbody>
                {filteredStudents?.map((student) => (
                  <tr key={student.id} className="border-b border-[#F0EDE8] hover:bg-[#FAFAF8]">
                    ...
                    <td className="py-3 px-4 text-center text-[#6B6560]">{student.className} {student.section && `- ${student.section}`}</td>
                    <td className="py-3 px-4 text-center text-[#6B6560] capitalize">{student.gender}</td>
                    <td className="py-3 px-4 text-[#6B6560] text-xs">{student.schoolName || "-"}</td>
                  </tr>
                ))}
                {(!filteredStudents || filteredStudents.length === 0) && (
                  <tr>
                    <td colSpan={4} className="py-8 text-center text-[#9B9590]">
