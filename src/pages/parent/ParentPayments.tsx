            ...
            <table className="w-full text-sm">
              ...
              <tbody>
                {payments?.map((payment) => (
                  <tr key={payment.id} className="border-b border-[#F0EDE8] hover:bg-[#FAFAF8]">
                    ...
                    <td className="py-3 px-4 text-right">
                      {payment.status === "PAID" && (
                        <Button variant="outline" size="sm" className="text-xs border-[#D9D4CC] hover:bg-[#F0EDE8]" onClick={() => alert("PDF receipt - Phase 2 feature")}>
                ...
                {(!payments || payments.length === 0) && (
                  <tr>
                    <td colSpan={5} className="py-8 text-center text-[#9B9590]">No payments yet</td>
                  </tr>
                )}
              </tbody>
            </table>
